...
describe Howcast::Client, "search" do
  ...
  it "should append mode=extended when passed in as an option" do
    @hc.should_receive(:open).with("http://www.howcast.com/search.xml?q=something&mode=extended&view=videos&api_key=myapikey").and_return(videos_xml)
...
describe Howcast::Client, "advanced_search" do
  ...
  it "should establish a connection to the search path on howcast.com" do
    @hc.should_receive(:open).with(%r{^http://www.howcast.com/search.xml}).and_return(videos_xml)
    @hc.advanced_search({})
  end

  it "should url-encode any passed params" do
    @hc.should_receive(:open).with(%r{q=something}).and_return(videos_xml)
    @hc.advanced_search :q => "something"
  end

  it "should not create default params" do
    @hc.should_receive(:open).with(%r{mode=foo}).and_return(videos_xml)
    @hc.should_not_receive(:open).with(%r{view=})
