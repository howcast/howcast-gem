...
describe Howcast::Client, "category" do  
  ...
  it "should establish a connection with the correct category url" do
    @hc.should_receive(:open).with(equivalent_uri("http://api.howcast.com/categories/2.xml?api_key=myapikey")).and_return(category_xml)
    @hc.category(2)
  end  
...
describe Howcast::Client, "categories" do
  ...
  it "should establish a connection with categories.xml" do
    @hc.should_receive(:open).with(equivalent_uri("http://api.howcast.com/categories.xml?api_key=myapikey")).and_return(categories_xml)
    @hc.categories
  end
